...
def _parse_ply_header(f) -> Tuple[str, int, List[Tuple[str, str]], int]:
    ...
    if first != "ply":
        raise ValueError("Invalid PLY file: missing 'ply' magic header")
    ...
    while True:
        raw = f.readline()
        if not raw:
            raise ValueError("Invalid PLY file: unexpected EOF in header")
        header_lines += 1
        ...
        if tokens[0] == "format":
            if len(tokens) < 2:
                raise ValueError("Invalid PLY header: malformed format line")
            fmt = tokens[1]
        elif tokens[0] == "element":
            if len(tokens) != 3:
                raise ValueError("Invalid PLY header: malformed element line")
            in_vertex_element = tokens[1] == "vertex"
            if in_vertex_element:
                num_vertices = int(tokens[2])
        elif tokens[0] == "property" and in_vertex_element:
            if len(tokens) == 3:
                prop_type, prop_name = tokens[1], tokens[2]
                ...
                # Skip list properties for vertices if present.
                continue

    if fmt is None:
        raise ValueError("Invalid PLY header: missing format")
    if num_vertices is None:
        raise ValueError("Invalid PLY header: missing vertex element")
    if not vertex_props:
        raise ValueError("Invalid PLY header: no vertex properties found")

    return fmt, num_vertices, vertex_props, header_lines


def load_ply_points_rgb(ply_path: str) -> Tuple[np.ndarray, np.ndarray]:
    with open(ply_path, "rb") as f:
        fmt, num_vertices, vertex_props, _ = _parse_ply_header(f)
        prop_names = [name for name, _ in vertex_props]

        for required in ["x", "y", "z"]:
            if required not in prop_names:
                raise ValueError(f"PLY is missing required vertex property: {required}")
        ...
        if fmt == "ascii":
            ...
            for _ in range(num_vertices):
                ...
                rows.append(vals[: len(vertex_props)])
            if len(rows) != num_vertices:
                raise ValueError(
                    f"Invalid ASCII PLY: expected {num_vertices} vertices, got {len(rows)}"
                )

            data = {}
            for i, (name, ptype) in enumerate(vertex_props):
                np_dtype = np.dtype(_PLY_DTYPE_MAP[ptype])
                data[name] = np.asarray([row[i] for row in rows], dtype=np_dtype)

        elif fmt == "binary_little_endian":
            dtype = np.dtype(
                [(name, "<" + _PLY_DTYPE_MAP[ptype]) for name, ptype in vertex_props]
            )
            data_np = np.fromfile(f, dtype=dtype, count=num_vertices)
            if data_np.shape[0] != num_vertices:
                raise ValueError(
                    f"Invalid binary PLY: expected {num_vertices} vertices, got {data_np.shape[0]}"
                )
            data = {name: data_np[name] for name, _ in vertex_props}
        else:
            raise ValueError(
                f"Unsupported PLY format: {fmt}. Supported: ascii, binary_little_endian"
            )

    points = np.stack([data["x"], data["y"], data["z"]], axis=1).astype(np.float32)

    rgb_keys = None
    if all(k in data for k in ["red", "green", "blue"]):
        rgb_keys = ("red", "green", "blue")
    elif all(k in data for k in ["r", "g", "b"]):
        rgb_keys = ("r", "g", "b")

    if rgb_keys is None:
        rgbs = np.full((points.shape[0], 3), 127, dtype=np.uint8)
    else:
        raw_rgb = np.stack([data[rgb_keys[0]], data[rgb_keys[1]], data[rgb_keys[2]]], axis=1)
        if np.issubdtype(raw_rgb.dtype, np.floating):
            if raw_rgb.max() <= 1.0:
                raw_rgb = raw_rgb * 255.0
            rgbs = np.clip(raw_rgb, 0.0, 255.0).astype(np.uint8)
        else:
            rgbs = np.clip(raw_rgb, 0, 255).astype(np.uint8)

    if points.shape[0] == 0:
        raise ValueError("PLY has zero vertices")
    return points, rgbs
...
def random_downsample_points(
    points: np.ndarray, rgbs: np.ndarray, max_points: int, seed: int = 42
) -> Tuple[np.ndarray, np.ndarray]:
    if max_points <= 0:
        raise ValueError("lidar_max_points must be > 0")
    if points.shape[0] <= max_points:
        return points, rgbs
    rng = np.random.default_rng(seed)
    inds = rng.choice(points.shape[0], size=max_points, replace=False)
    return points[inds], rgbs[inds]
...
class Config:
    ...
    init_type: str = "sfm"
    # Optional aligned LiDAR point cloud (.ply) used instead of COLMAP sparse points.
    lidar_ply: Optional[str] = None
    # Optional max number of LiDAR points to keep (random downsample).
...
class Runner:
    ...
    def __init__(
        ...
        if cfg.lidar_ply is not None:
            lidar_ply_path = cfg.lidar_ply
            if not os.path.isabs(lidar_ply_path):
                lidar_ply_path = os.path.join(cfg.data_dir, lidar_ply_path)
            if not os.path.exists(lidar_ply_path):
                raise FileNotFoundError(f"LiDAR PLY not found: {lidar_ply_path}")

            lidar_points, lidar_rgbs = load_ply_points_rgb(lidar_ply_path)
            num_raw = lidar_points.shape[0]

            # Match parser coordinate system after optional COLMAP normalization.
            if cfg.normalize_world_space:
                lidar_points = apply_transform_to_points(lidar_points, self.parser.transform)

            if cfg.lidar_max_points is not None:
                lidar_points, lidar_rgbs = random_downsample_points(
                    lidar_points, lidar_rgbs, cfg.lidar_max_points
                )

            self.parser.points = lidar_points.astype(np.float32)
            self.parser.points_rgb = lidar_rgbs.astype(np.uint8)
            self.parser.points_err = np.ones((self.parser.points.shape[0],), dtype=np.float32)

            print(
                f"[LiDAR] Loaded {num_raw} points from {lidar_ply_path}; using {self.parser.points.shape[0]} points for initialization."
            )
